                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        ...
                        <span>
                          {new Date(order.orders.date).toLocaleDateString('es-ES')} - {order.orders.time}
                        </span>
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <Badge className={`${statusColors[order.orders.status]} border-0`}>
                        {order.orders.status === 'in_progress' ? 'En Proceso' : 
                         order.orders.status === 'ready' ? 'Listo' :
                         order.orders.status === 'delivered' ? 'Entregado' :
                         order.orders.status === 'cancelled' ? 'Cancelado' : 'Pendiente'}
                      </Badge>
                      <Badge variant="outline" className={`${paymentColors[order.orders.paymentStatus]} border-0`}>
                        {order.orders.paymentStatus === 'partial' ? 'Parcial' : 
                         order.orders.paymentStatus === 'paid' ? 'Pagado' : 'Pendiente'}
                      </Badge>
